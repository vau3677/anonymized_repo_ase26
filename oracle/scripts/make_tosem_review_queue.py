...
def summarize_join_row(row) -> str:
    return " | ".join([
...
def build_review_template(summary_rows: list[dict[str, str]], join_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in join_rows:
        if row.get("mvbench_status") == "finding_match": joined_by_oracle[row["oracle_id"]].append(row)

    review_rows = []
    for row in summary_rows:
        if row["mvbench_status"] not in {"finding_match", "repo_sheet_no_finding"}: continue
        ...
        out["annotator_1_semantic_label"] = "unlabeled"
        out["annotator_1_mvsi_subtype"] = "NA"
        out["annotator_1_strict_b0_match"] = "not_checked"
...
def main():
    parser = argparse.ArgumentParser(
        description=("Builds a blank TOSEM review template from scripts/overlap.py outputs. This is not the canonical reviewed queue.")
    )
    parser.add_argument("--summary", default="oracle/tosem_web3bugs_mvbench_summary.csv")
    parser.add_argument("--join", default="oracle/tosem_web3bugs_mvbench_join.csv", help="Transient detailed join CSV produced by oracle/scripts/overlap.py.")
    parser.add_argument("--out", default="oracle/tosem_oracle_review_queue_template.csv")
    ...
    if not join_path.exists():
        raise SystemExit(
            "Missing transient join CSV: "
            f"{join_path}. Regenerate it first with: "
